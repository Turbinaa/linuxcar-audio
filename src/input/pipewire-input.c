...
void pw_input_main_loop() {
    ...
    data.stream = pw_stream_new_simple(pw_main_loop_get_loop(data.loop), "Capture audio", props,
                                       &stream_e, &data);
    params[0] = spa_format_audio_raw_build(
        &b, SPA_PARAM_EnumFormat, &SPA_AUDIO_INFO_RAW_INIT(.format = SPA_AUDIO_FORMAT_F32));
    pw_stream_connect(
...
void stream_param_changed(void *_data, uint32_t id, const struct spa_pod *p) {
